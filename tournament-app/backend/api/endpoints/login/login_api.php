...
try {
    ...
    if ($metodo === 'POST') {
        ...
        switch ($action) {
            ...
            case 'cerrar-sesion':
                if (!isset($datos['id_sesion'])) apiError('Falta id_sesion');
            ...
            case 'crear-usuario':
                ...
                if ($resultado) apiResponse($resultado, 'Usuario creado', 201);
        ...
        switch ($action) {
            case 'listar-usuarios':
                ...
                break;
            case 'buscar-usuario':
                if (!isset($_GET['id'])) apiError('ID requerido');
        ...
        switch ($action) {
            case 'actualizar-usuario':
                if (!isset($datos['id_usuario'])) apiError('ID de usuario requerido');
                $resultado = $usuarioController->actualizarInformacion($datos);
                if ($resultado) apiResponse(null, 'Usuario actualizado');
            ...
            case 'actualizar-password':
                ...
                if ($resultado) apiResponse(null, 'Password actualizado');
        ...
        switch ($action) {
            case 'eliminar-usuario':
                ...
                if ($resultado) apiResponse(null, 'Usuario eliminado');
